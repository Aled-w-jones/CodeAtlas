...
<div class="annotation-panel h-full flex flex-col">
	{#if walkthroughMode && walkthrough}
		<!-- Walkthrough Header -->
		<div class="walkthrough-header p-4 border-b border-vsc-border-light bg-vsc-bg-light">
			<h3 class="text-sm font-semibold text-vsc-text-primary mb-1">
				🎓 {walkthrough.title}
			</h3>
			<p class="text-xs text-vsc-text-secondary mb-2">
	...
	<div class="flex-1 overflow-y-auto">
					...
					<div class="annotation-card border rounded-lg p-4 {getPriorityClass(annotation.priority)}">
						...
						<div class="flex items-start gap-2 mb-2">
							...
							<div class="flex-1 min-w-0">
								<h4 class="font-semibold text-sm text-vsc-text-primary break-words">
									{annotation.title}
								</h4>
								{#if annotation.lines}
									<div class="text-xs text-vsc-text-secondary mt-1">
							...
							<span class="text-xs px-2 py-1 rounded bg-vsc-bg-medium text-vsc-text-secondary flex-shrink-0">
								{annotation.type}
							</span>
						</div>
						
						<!-- Annotation Content -->
						<div class="content text-sm text-vsc-text-secondary leading-relaxed">
						...
						{#if annotation.tags && annotation.tags.length > 0}
							<div class="flex flex-wrap gap-1 mt-3">
								{#each annotation.tags as tag}
									<span class="text-xs px-2 py-1 rounded bg-vsc-bg-light text-vsc-text-secondary">
										{tag}
									</span>
								{/each}
							</div>
		...
		{:else if walkthroughMode}
			...
			<div class="p-4 text-center">
				...
				<p class="text-sm text-vsc-text-secondary">
