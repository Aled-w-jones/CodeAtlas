<script>
  ...
  import WalkthroughMode from './WalkthroughMode.svelte';
  ...
  export let notepadData = null;
  
  // Extract data from props like MK1
  $: ({ notepad, content, readmeContent, rawReadmeContent, annotations } = notepadData || {});
  
  // Component state matching MK1
  let codeElement;
  let codeContainer;
...
{:else if error}
  <div class="container mx-auto px-4 py-8">
    <div class="text-center py-12">
      ...
      <a href={getPath("/notepads")} class="text-vsc-accent-blue hover:underline mt-4 inline-block">&larr; Back to Notepads</a>
    </div>
  </div>
{:else if notepad}
<div class="container mx-auto px-4 py-4 lg:py-8 max-w-none page-container" class:loaded={pageLoaded}>
  <div class="mb-6">
    <a href={getPath("/notepads")} class="text-vsc-accent-blue hover:underline">&larr; Back to Notepads</a>
  </div>
  
  <!-- Header Section -->
  <header class="mb-6 lg:mb-8">
    <h1 class="text-xl lg:text-3xl font-bold mb-3 lg:mb-4 text-vsc-text-primary break-words">{notepad.title}</h1>
    <p class="text-vsc-text-secondary mb-3 lg:mb-4 text-sm lg:text-base">{notepad.description}</p>
    
    <div class="flex flex-wrap gap-1 lg:gap-2 mb-3 lg:mb-4">
      {#each notepad.tags as tag}
        <span class="bg-vsc-bg-light text-vsc-text-secondary px-2 py-1 rounded text-xs lg:text-sm">
          {tag}
        </span>
      {/each}
    </div>
    
    <div class="flex flex-col lg:flex-row lg:flex-wrap gap-2 lg:gap-4 text-xs lg:text-sm text-vsc-text-secondary mb-3 lg:mb-4">
  ...
  {#if !isMobile}
    ...
    <div bind:this={splitContainer} class="split-view flex flex-col lg:flex-row h-[calc(100vh-16rem)] {isResizing ? 'select-none' : ''}">
      ...
      {#if showAnnotations && annotations?.walkthrough}
        <div class="walkthrough-container bg-vsc-bg-medium border border-vsc-border-light rounded-lg overflow-hidden mr-2" style="width: 300px; flex-shrink: 0;">
          <WalkthroughMode 
      ...
      <!-- Code Panel -->
      <div 
        class="bg-vsc-bg-medium border border-vsc-border-light rounded-lg overflow-hidden flex flex-col transition-all duration-300"
      ...
      >
        <div class="bg-vsc-bg-light px-4 py-2 border-b border-vsc-border-light flex-shrink-0">
          <span class="text-vsc-text-secondary text-sm">🐍 {notepad.filePath}</span>
        </div>
    ...
    {#if showReadme && readmeContent}
      ...
      >
        <div class="bg-vsc-bg-light px-4 py-2 border-b border-vsc-border-light flex-shrink-0 flex items-center justify-between">
          {#if !readmeMinimized}
            <span class="text-vsc-text-secondary text-sm">📖 Documentation</span>
          ...
          {:else}
            <button 
              on:click={toggleReadmeMinimize}
              class="text-vsc-text-secondary hover:text-vsc-text-primary transition-colors text-sm rotate-90 mx-auto"
              title="Expand panel"
            >
              📖
            </button>
        ...
        {#if !readmeMinimized}
          <div class="flex-1 overflow-auto">
            {#if !readmeCollapsed}
              <div class="p-4">
                {#if showRawReadme}
                  <pre class="!bg-vsc-bg-dark !border border-vsc-border-light !rounded m-0 p-3 text-xs leading-relaxed overflow-auto"><code class="language-markdown">{rawReadmeContent || readmeContent}</code></pre>
                {:else}
            ...
            {:else}
              <div class="p-4 text-center text-vsc-text-secondary">
    ...
    {#if showAnnotations && annotations}
      ...
      <div 
        class="annotation-panel-container bg-vsc-bg-medium border border-vsc-border-light rounded-lg overflow-hidden flex flex-col"
        style="width: {annotationPanelWidth}%; min-width: 250px;"
      >
        <div class="bg-vsc-bg-light px-4 py-2 border-b border-vsc-border-light flex-shrink-0">
          <span class="text-vsc-text-secondary text-sm">💡 Code Annotations</span>
        </div>
        <AnnotationPanel 
  ...
  {:else}
    <!-- Mobile View -->
    <div class="bg-vsc-bg-medium border border-vsc-border-light rounded-lg overflow-hidden">
      <div class="bg-vsc-bg-light px-4 py-2 border-b border-vsc-border-light">
        <span class="text-vsc-text-secondary text-sm">🐍 {notepad.filePath}</span>
      </div>
      {#if readmeContent}
        <div class="p-4 text-center">
          <p class="text-vsc-text-secondary mb-4">
            This notepad has documentation available. Use the buttons above to view code or documentation.
          </p>
          <div class="flex gap-3 justify-center">
            <button 
              on:click={showMobileCode}
              class="bg-vsc-accent-blue text-white px-6 py-3 rounded hover:bg-blue-600 transition-colors"
            >
              💻 View Code
            </button>
            <button 
              on:click={showMobileDocumentation}
              class="bg-vsc-bg-light text-vsc-text-primary px-6 py-3 rounded hover:bg-vsc-bg-dark transition-colors border border-vsc-border-light"
            >
              📖 View Documentation
            </button>
          </div>
      ...
      {:else}
        ...
        <div class="overflow-auto">
          <pre class="!bg-vsc-bg-medium !border-0 !rounded-none m-0"><code 
            bind:this={codeElement}
...
{#if showMobileModal && isMobile}
  <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
    <div class="bg-vsc-bg-medium border border-vsc-border-light rounded-lg w-full max-w-4xl h-[90vh] flex flex-col">
      ...
      <!-- Modal Footer -->
      <div class="bg-vsc-bg-light px-4 py-3 border-t border-vsc-border-light flex gap-2 flex-shrink-0">
        <button 
          on:click={() => {
            mobileModalContent = 'code';
          ...
          class="px-4 py-2 rounded transition-colors {mobileModalContent === 'code' ? 'bg-vsc-accent-blue text-white' : 'bg-vsc-bg-medium text-vsc-text-primary border border-vsc-border-light'}"
        >
          💻 Code
        </button>
        <button 
          on:click={() => mobileModalContent = 'documentation'}
          class="px-4 py-2 rounded transition-colors {mobileModalContent === 'documentation' ? 'bg-vsc-accent-blue text-white' : 'bg-vsc-bg-medium text-vsc-text-primary border border-vsc-border-light'}"
        >
          📖 Documentation
        </button>
        ...
        <button 
          on:click={closeMobileModal}
          class="bg-vsc-bg-medium text-vsc-text-primary px-4 py-2 rounded hover:bg-vsc-bg-light transition-colors border border-vsc-border-light"
        >
          Close
        </button>
      </div>
...
<style>
  ...
  :global(.prose blockquote) {
    border-left: 4px solid theme('colors.vsc-accent-blue');
    padding-left: 1rem;
    margin: 1rem 0;
    font-style: italic;
    color: theme('colors.vsc-text-secondary');
